...
Calendar.prototype = {
    ...
    init: function () {
        ...
        el.on("click", ".calendar-year-select", function (e) {
            ...
            var s = '';
			console.log($('.calendar-year-text').text())
			var clickY = Number($('.calendar-year-text').text().split('年')[0])
			var clickH = (clickY  - 1950) * 30
			$('#dropdown-year').animate({scrollTop:clickH - 80},10)
			console.log($('.year-item').length)
            ...
            for (var i = 0; i < 100; i++) {
                ...
                // }
				s += '<li class="year-item">'
                s += '<span class="year-check">' + (yearText - 70 + i) + '</span>'
                s += '<span >年</span>'
                s += '</li>'
            }
            me.el.find(".dropdown-year").html(s);
			for(var j = 0 ; j < $('.year-item').length ; j++){
				if($('.year-item').eq(j).find('.year-check').text() == clickY){
					$('.year-item').eq(j).addClass('active')
				}
			}
        })
        //年份改变
		...
		//月份向前点击
		el.on("click", ".leftmonth", function (e) {
		    e.stopPropagation();
		    $(".dropdown-month").removeClass("open");
		    var monthText = $('.calendar-month-text').text();
		    var monthNum = parseInt(monthText.split("月")[0]);
			console.log(opts.newDate)
			if(monthNum == 1){
				var thisyear = $('.calendar-year-text').text()
				var thisyearnumber = parseInt(thisyear.split("年")[0])
				$('.calendar-year-text').text(thisyearnumber - 1 + '年')
				monthNum = 13
			}
		    opts.newDate.setMonth(opts.newDate.getMonth() - 1);
		    me._refreshCalendar(opts.newDate);
		    $(".calendar-month-text").text(monthNum-1 + '月');
			
		})
		//月份向后点击
		el.on("click", ".rightmonth", function (e) {
		    e.stopPropagation();
		    $(".dropdown-month").removeClass("open");
		    var monthText = $('.calendar-month-text').text();
		    var monthNum = parseInt(monthText.split("月")[0]);
    ...
    _createToolbar: function () {
        ...
        if (mode == "month") {
            ...
            s += '<span class="leftmonth"><&nbsp;</span><div class="calendar-select  calendar-month-select">'
            ...
            s += '</ul>'
            s += '</div ><span class="rightmonth">></span> '
        }
    ...
    _refreshCalendar: function (newDate) {
        ...
        for (var i = 0; i < 6; i++) {
            ...
            for (var l = 0; l < 7; l++) {
				...
				var titleMonth = renderDate.getMonth() + 1 < 10 ? '0' + (renderDate.getMonth() + 1) : renderDate.getMonth() + 1
				var titleDate = renderDate.getDate() < 10 ? '0' + renderDate.getDate() : renderDate.getDate();
				

                if (renderDate.getMonth() < newDate.getMonth()) {
                    s += '<td title="' + year + '-' + titleMonth + '-' + titleDate + '" class="calendar-cell calendar-last-month-cell">';
                }
                else if (renderDate.getMonth() > newDate.getMonth()) {
                    s += '<td title="' + year + '-' + titleMonth + '-' + titleDate + '" class="calendar-cell calendar-next-month-cell">';
                }
                else if (date == nowNum) {
                    s += '<td title="' + year + '-' + titleMonth + '-' + titleDate + '" class="calendar-cell calendar-today">';
                }
                else {
                    s += '<td title="' + year + '-' + titleMonth + '-' + titleDate + '" class="calendar-cell">';
                }
                s += '<div class="calendar-date">';
